<template>
  <div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
      ...
      <h1 class="text-2xl font-bold text-white">{{ isEditing ? 'Modifier le produit' : 'Nouveau produit' }}</h1>
    </div>
    ...
    <form v-else @submit.prevent="saveProduct" class="gfg-surface p-6 sm:p-8 rounded-xl space-y-8">
...
const isEditing = computed(() => !!productId && productId !== 'add' && productId !== 'new') // Assuming 'add' or 'new' isn't used here, but good to be safe. Actually, [id] matches everything.
...
onMounted(async () => {
  try {
    // Fetch categories
    const cats = await $fetch<Category[]>(`${config.apiBase}/categories`)
    categories.value = cats

    if (isEditing.value) {
      const product = await $fetch<Product>(`${config.apiBase}/products/${productId}`)
      if (product) {
        Object.assign(form, {
          ...product,
...
const saveProduct = async () => {
  ...
  try {
    const url = isEditing.value ? `${config.apiBase}/products/${productId}` : `${config.apiBase}/products`
    ...
    toast.add({
      title: isEditing.value ? 'Produit modifié avec succès' : 'Produit créé avec succès',
      color: 'green'
    })
    router.push('/admin/products')
  } catch (e: any) {
    ...
    toast.add({ 
      title: isEditing.value ? 'Erreur lors de la modification' : 'Erreur lors de la création', 
